/* ---------- headers */


/* ---------- constants */

/* ---------- definitions */

/* ---------- prototypes */

/* ---------- globals */

/* ---------- public code */

/* ---------- private code */

/* ---------- reverse engineering */

// private: static class c_assert_handler *c_assert_handler_hook::g_assert_handler; // "?g_assert_handler@c_assert_handler_hook@@0PAVc_assert_handler@@A"

// public: static void c_assert_handler_hook::attach_handler(class c_assert_handler *);
// public: static void c_assert_handler_hook::detach_handler(class c_assert_handler *);
// public: static bool c_assert_handler_hook::active(void);
// public: static class c_assert_handler * c_assert_handler_hook::get_handler(void);

//public: static void c_assert_handler_hook::attach_handler(class c_assert_handler *)
//{
//    mangled_ppc("?attach_handler@c_assert_handler_hook@@SAXPAVc_assert_handler@@@Z");
//};

//public: static void c_assert_handler_hook::detach_handler(class c_assert_handler *)
//{
//    mangled_ppc("?detach_handler@c_assert_handler_hook@@SAXPAVc_assert_handler@@@Z");
//};

//public: static bool c_assert_handler_hook::active(void)
//{
//    mangled_ppc("?active@c_assert_handler_hook@@SA_NXZ");
//};

//public: static class c_assert_handler * c_assert_handler_hook::get_handler(void)
//{
//    mangled_ppc("?get_handler@c_assert_handler_hook@@SAPAVc_assert_handler@@XZ");
//};

